...
class Captcha(object):
    ...
    def display(self):
        return HttpResponse(self.__display(), 'image/gif')
    ...
    def save(self):
        # filename = 'captcha' + random.choice( ['apple', 'pear', 'peach', 'orange', 'lemon'] ) + '.gif'
        filename = 'captcha' + str(int(time.time())) + '.gif'
        if settings.ENV=='SAE':
            return delThenSetToSAEStorage(filename, self.__display());
        elif settings.ENV=='VM':
            localstorage = NeoStorage()
            localstorage.delete('captcha/')
            return localstorage.saveB('captcha/'+filename, self.__display())
    
    def getCode(self):
        return  self.django_request.session[self.session_key];
